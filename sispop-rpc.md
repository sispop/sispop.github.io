...
* _unlocked\_balance_  - unsigned int; Unlocked funds are those funds that are sufficiently deep enough in the Sispop blockchain to be considered safe to spend.
...
* _unlock\_time_  - unsigned int; Number of blocks before the $SISPOP can be spent \(0 to not add a lock\).
...
* _do\_not\_relay_  - boolean; \(Optional\) If true, the newly created transaction will not be relayed to the Sispop network. \(Defaults to false\)
